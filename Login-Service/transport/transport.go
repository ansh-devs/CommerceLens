...
func NewHttpServer(ctx context.Context, endpoints *endpoints.Endpoints) http.Handler {
	...
	r.Methods("POST").Path("/v1/register").Handler(transport.NewServer(
		endpoints.RegisterUser,
		JsonRegisterUserRequestDecoder,
		JsonResponseEncoder,
	))

	r.Methods("POST").Path("/v1/login").Handler(transport.NewServer(
		endpoints.LoginUser,
		JsonLoginUserRequestDecoder,
		JsonResponseEncoder,
	))

	r.Methods("GET").Path("/v1/get-user/{id}").Handler(transport.NewServer(
